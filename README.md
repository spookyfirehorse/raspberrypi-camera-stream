...
PULSE_LATENCY_MSEC=10 chrt -f 90 taskset -c 3   ffmpeg -realtime 1 -y -loglevel warning  -hwaccel drm -hwaccel_device /dev/dri/renderD128  \
-fflags +genpts+igndts+nobuffer+flush_packets  \
-use_wallclock_as_timestamps 1  \
...
--libav-video-codec h264_v4l2m2m --libav-format h264 --codec libav --inline \
--awb indoor --profile baseline --intra 25 -b 1500000 -n -o - | \
nice -n -11 ffmpeg -realtime 1 -y -fflags +genpts+igndts+nobuffer+flush_packets \
-use_wallclock_as_timestamps 1 \
-thread_queue_size 32 -f h264 -r 25 -i - \
-thread_queue_size 128 -f pulse -fragment_size 512 -isync 0 -i default \
-c:v copy -metadata title='lucy' \
...
--inline  -n  -o  - | chrt -f 90 taskset -c 3  ffmpeg -realtime 1 -loglevel warning  -hide_banner -fflags nobuffer+genpts+flush_packets \
